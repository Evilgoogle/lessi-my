...
/**
 * Description of ProcessDistribution
 *
 * @author Evilgoogle
 */
class ProcessDistribution
{
    private $ready_users = [0];
    private $leads = [];
    ...
    private function testUsers($date)
    {
        $users = LeadUser::all();
        ...
        foreach($this->ready_users as $k=>$user) {
            $isset = User2Lead::select('lead_auto_users2leads.user_id', 'lead_auto_users2leads.lead_id')
                ->join('lead_autos as la', 'la.id', '=', 'lead_auto_users2leads.lead_id')
                ->where('lead_auto_users2leads.user_id', $user)
                ->where('la.complete', 'not completed')
                ->first();
    ...
    private function makeLeads()
    {
        ...
        // Проверка - не отдан ли лид пользователю
        foreach($leads as $item) {
            $isset = User2Lead::where('lead_id', $item->id)->first();
    ...
    {
        $distrib = new Distrib($this->leads, $this->ready_users);
        $distrib->run();
    ...
    {
        $send = new MailSender('new-lead', ['3devilmax@gmail.com']);
        $send->run();
    }
}